...
```r
train<-read.csv("C:/Users/Admin/Downloads/pml-training.csv")
test<-read.csv("C:/Users/Admin/Downloads/pml-testing.csv")
```
...
#### LDA Model
```r
lda_model<-train(classe~ ., data=training, method="lda", trControl = control)
set.seed(200)
predict<-predict(lda_model,testing)
confusionMatrix(factor(predict,levels = 1:148),factor(testing$classe,levels = 1:148))
```
